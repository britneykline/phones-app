...
class CamerasControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create camera" do
    assert_difference('Camera.count') do
      post cameras_url, params: { camera: { megapixel: @camera.megapixel, name: @camera.name, size: @camera.size } }
    end
  ...
  test "should update camera" do
    patch camera_url(@camera), params: { camera: { megapixel: @camera.megapixel, name: @camera.name, size: @camera.size } }
